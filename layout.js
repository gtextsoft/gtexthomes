(function () {
  const cfg = typeof GTEXT_CONFIG !== "undefined" ? GTEXT_CONFIG : {};
  ...
  const currentSlug = currentPath
    ...
    .replace(/\.html$/, "")
    .replace(/^index$/, "");
  const isHome = currentSlug === "" || currentPath === "/";

  function isActive(href) {
    const slug = href.replace(/^\//, "").replace(/\.html$/, "").replace(/^index$/, "");
    if (slug === "" || slug === "index") return isHome;
  ...
  function renderFooter() {
    ...
    const footerSocials = socials
      .map(
        (s) =>
          `<a href="${s.url}" class="social-link" target="_blank" rel="noopener noreferrer" aria-label="${s.name}">${socialIcon(s.icon)}</a>`
      )
      .join("");
      ...
      <footer class="site-footer">
        <div class="container">
          <div class="footer-grid">
            ...
            <div class="footer-links">
              ...
              <ul>
                ...
                <li><a href="/properties.html#land">Land Acquisition</a></li>
          ...
          <div class="footer-bottom">
            <p>&copy; ${new Date().getFullYear()} GText Homes. All rights reserved.</p>
          </div>
        </div>
      </footer>`;
  }
  ...
  function renderWhatsApp() {
    ...
    btn.href = `https://wa.me/${contact.phoneRaw}?text=${encodeURIComponent("Hello GText Homes, I would like to inquire about your properties.")}`;
    btn.target = "_blank";
  ...
  function injectGA4() {
    ...
    s2.textContent = `window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag('js',new Date());gtag('config','${cfg.ga4Id}');`;
